...
| 영역 | 하는 일 |
| --- | --- |
| 세션 | omp·Claude Code·Codex CLI·Gemini CLI·pi의 작업 세션을 고른다. 검색하거나 최근 10개 다음 목록을 더 볼 수 있다. |
| 작업 내용 | AI의 질문·결과·계획을 최신 순서로 보여 준다. 준비된 설명의 제목·요약을 보고 카드를 누른다. 오른쪽 경계를 끌어 너비를 바꿀 수 있다. |
| 카드별 대화 | 선택한 카드에 관해 이어 묻는다. 대화 맨 위에 그 카드의 AI 출력 내용이 먼저 보인다. 앞 문답을 기억하는 이어지는 대화이고, 질문이 다른 카드를 가리키면 그 카드 대화로 옮기기를 제안한다. 카드와 무관한 질문은 **세션 전체 대화**에 남긴다. 답은 나오는 대로 보이고, 결론부터 보여 주며 자세한 설명은 접어 둔다. |
...
| 설정 | 프로필별 도슨트 모델, 기본·영역별 설명 모드, 미리 설명, 밝음·어둠·자동 테마를 고른다. |
