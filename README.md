...
- Claude Code 세션은 선택 사항이다. 읽을 수 있는 세션이 있으면 omp 세션과 함께 목록에 나온다.
...
| 영역 | 하는 일 |
| --- | --- |
| 세션 | omp·Claude Code의 작업 세션을 고른다. 검색하거나 최근 10개 다음 목록을 더 볼 수 있다. |
| 작업 내용 | AI의 질문·결과·계획을 최신 순서로 보여 준다. 준비된 설명의 제목·요약을 보고 카드를 누른다. 오른쪽 경계를 끌어 너비를 바꿀 수 있다. |
| 카드별 대화 | 선택한 카드에 관해 이어 묻는다. 카드와 무관한 질문은 **세션 전체 대화**에 남긴다. 답은 결론부터 보여 주고 자세한 설명은 접어 둔다. |
...
| 설정 | 프로필별 도슨트 모델, 기본·영역별 설명 모드, 자동 설명, 밝음·어둠·자동 테마를 고른다. |

![가상 문답의 핵심 단어를 모은 배운 내용 다시보기](docs/assets/screenshot-review.png)

새 중요 사건의 쉬운 설명은 기본적으로 미리 준비하지만, 명시적으로 끈 설정은 유지한다. 한 번에 하나씩, 10분에 최대 5개다. 모델 호출에 시간과 비용이 들며 이전 사건은 자동 설명하지 않는다. 선택한 도슨트 모델은 *새 설명 요청*에만 적용되고, 원래 작업 세션의 모델을 바꾸지 않는다. 표시된 입력·출력 단가는 omp 카탈로그의 참고값이지 실제 청구액이 아니다.
...
보여 주는 컴퓨터에는 `{"host":"tailscale"}`로 설정할 수도 있다. 원격 세션의 전사와 라이브를 읽고 **내 컴퓨터의 omp**로 답을 만든다. 원격 문답 기록은 프로필이 같은 사람인지 확인할 수 없어 자동으로 합치지 않는다. 양쪽 도슨트 버전을 맞추는 편이 안전하다.
